...
class PunipuniTicketModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title="ぷにぷに代行 - チケット情報")
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        channel_name = f"ぷにぷに代行-{user.name}"
...
async def setup(bot: commands.Bot):
    @bot.tree.command(name="punipuni_ticket", description="ぷにぷに代行チケット作成")
    async def punipuni_ticket(interaction: discord.Interaction):
        modal = PunipuniTicketModal()
